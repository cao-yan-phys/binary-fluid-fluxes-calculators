...
Numerical calculators for the time-averaged sound-wave fluxes generated by bound Keplerian binaries in homogeneous fluid backgrounds. The supported media are a classical inviscid Newtonian barotropic fluid and a Schrödinger-Poisson quantum fluid.

The full numerical calculators evaluate the harmonic sums with automatic convergence checks. They support CUDA acceleration through `numba.cuda` when available and can also run on the CPU.
...
Consequently, the same dimensionless `n0` applies to both media:
...
- The full classical-fluid binary calculators check the large-harmonic divergence criterion before evaluating the sum.
- Please verify convergence settings for each new parameter regime.
- The quantum-fluid calculation uses a classical-field description.
