...
def point_source_speed_threshold_violation(
    nu: float,
    e: float,
    A: float,
    R1_over_a: float,
    ...
    q1, q2 = mass_fractions_from_nu(nu)
    orbital_factor = A * math.sqrt((1.0 + e) / (1.0 - e))
    return (
        (R1_over_a == 0.0 and q2 * orbital_factor >= 1.0)
        or (R2_over_a == 0.0 and q1 * orbital_factor >= 1.0)
    )
