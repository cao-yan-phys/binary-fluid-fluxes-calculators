...
class QuantumFluid:
    ...
    def __post_init__(self) -> None:
        ...
        if not math.isfinite(self.c_S_squared):
            raise ValueError("c_S_squared must be finite")

    @property
    def physical_k2_coefficient(self) -> float:
        return self.c_S_squared / (4.0 * self.m_phi**2)
    ...
    def denominator(self, omega: float, k: float) -> float:
        return self.a_n(omega) - self.physical_k2_coefficient * k * k - k**4 / (4.0 * self.m_phi**2)

    def roots(self, omega: float) -> tuple[float, float | None]:
        a_value = self.a_n(omega)
        coefficient = self.physical_k2_coefficient
        discriminant = math.sqrt(coefficient * coefficient + a_value / self.m_phi**2)
        k_squared = 2.0 * self.m_phi**2 * (discriminant - coefficient)
        kappa_squared = 2.0 * self.m_phi**2 * (discriminant + coefficient)
